...
config(){
    ...
    replace $app_home/config/application.yml defaultZone "http://127.0.0.1:$server_port/eureka" 1
    replace $app_home/config/application.yml name "$server_name" 1
    if [ ! -z "$eureka_instance_id" ];then
        replace $app_home/config/application.yml instance-id "$eureka_instance_id" 1
    fi

    replace $app_home/config/application.yml enabled "$mail_enable" 1
    replace $app_home/config/application.yml to "$mail_addr" 1

    replace $app_home/config/application.yml enabled "$dingtalk_enable" 2
    replace $app_home/config/application.yml webhook-url "$dingtalk_hook" 1
    replace $app_home/config/application.yml secret "$dingtalk_secret" 1
}
