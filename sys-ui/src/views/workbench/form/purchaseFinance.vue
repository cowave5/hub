  ...
  <el-dialog :title="title" :visible.sync="visible" width="65%" style="max-height: 80vh;">
    <el-form ref="form" :model="form" :rules="rules" label-width="150px" size="mini" style="float: right; width: 65%">
      ...
      <el-form-item label="审批结果">
        <el-radio-group v-model="financeResult" :disabled="task.complete">
      ...
      <el-form-item label="审批意见" prop="financeComment">
        <el-input v-model="form.financeComment" type="textarea" placeholder="..." style="width: 80%" :disabled="task.complete"/>
      </el-form-item>
    </el-form>
    ...
    getInfo(id) {
      infoPurchase(id).then( response => {
        ...
        if(this.task.complete){
          ...
          this.form.financeResult = this.form.processVariables.financeResult;
        }else{
          this.form.financeResult = true;
        }
      });
    },
    getRecords(id) {
    ...
    handleAgree(){
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if(this.financeResult){
            this.data.comment = "同意";
          }else{
            this.data.comment = "拒绝";
          }
          this.variables.financeComment = this.form.financeComment;
          this.variables.financeResult = this.financeResult;
