  ...
  <el-dialog :title="title" :visible.sync="visible" width="75%" style="max-height: 90vh;">
    <el-form ref="form" :model="form" :rules="rules" label-width="150px" size="mini" style="float: right; width: 65%">
      ...
      <el-form-item label="审批结果">
        <el-radio-group v-model="hrApproveResult" :disabled="task.complete">
          <el-radio :key="true" :label="true">同意</el-radio>
          <el-radio :key="false" :label="false">拒绝</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="审批意见" prop="hrComment">
        <el-input v-model="form.hrComment" type="textarea" placeholder="..." style="width: 80%" :disabled="task.complete"/>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    getInfo(id) {
      infoLeave(id).then( response => {
        ...
        if(this.task.complete){
          this.form.hrComment = this.form.processVariables.hrComment;
          this.form.hrApproveResult = this.form.processVariables.hrApproveResult;
        }else{
          this.form.hrApproveResult = true;
        }
      });
    },
    getRecords(id) {
    ...
    handleAgree(){
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if(this.hrApproveResult){
            this.data.comment = "同意";
          }else{
            this.data.comment = "拒绝";
          }
          this.variables.hrApproveResult = this.hrApproveResult;
          this.variables.hrComment = this.form.hrComment;
    ...
    /** 驳回 */
    handleReject(){
      instanceJump(this.task.taskId, 'leaveDept', '驳回').then( response => {
        this.visible = false;
        this.$emit('ok');
