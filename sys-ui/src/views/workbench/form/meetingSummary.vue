  ...
  <el-dialog :title="title" :visible.sync="visible" width="65%" style="max-height: 80vh;">
    <el-form ref="form" :model="form" :rules="rules" label-width="100px" style="float: right; width: 65%">
      ...
      <el-form-item label="会议纪要" prop="content">
        <el-input v-model="form.content" type="textarea" placeholder="..." style="width: 80%;" :rows="8" :disabled="task.complete"/>
      </el-form-item>
    </el-form>
    <div slot="footer" class="dialog-footer">
      <el-button v-if="!task.complete" type="primary" @click="handleSign">提交</el-button>
...
import {editMeeting, infoMeeting} from "@/api/workbench/meeting";
    ...
    handleSign(){
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.data.comment = "已提交";
          editMeeting(this.form).then( resp => {
            taskComplete(this.data).then( response => {
              this.visible = false;
              this.$emit('ok');
              this.$modal.msgSuccess("已提交纪要");
            });
          });
        }
      });
