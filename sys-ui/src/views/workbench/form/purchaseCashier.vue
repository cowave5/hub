<template>
  <el-dialog :title="title" :visible.sync="visible" width="65%" style="max-height: 80vh;">
    <el-form ref="form" :model="form" :rules="rules" label-width="150px" size="mini" style="float: right; width: 65%">
      ...
      <el-form-item v-if="form.money >= 10000" label="总经理意见" prop="generalComment">
        <el-input v-model="form.generalComment" type="textarea" placeholder="..." style="width: 80%" disabled="disabled"/>
      </el-form-item>
    </el-form>
    <div slot="footer" class="dialog-footer">
      <el-button v-if="!task.complete" type="primary" @click="handleAgree">付款</el-button>
    ...
    handleAgree(){
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.data.comment = "已付款";
          taskComplete(this.data).then( response => {
            this.visible = false;
            this.$emit('ok');
            this.$modal.msgSuccess("采购已付款");
          });
        }
      });
    }
