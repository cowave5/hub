...
config(){
    ## info.cluster
    replace $app_home/config/prod/application.yml id "$cluster_id" 1
    replace $app_home/config/prod/application.yml name "$cluster_name" 2
    replace $app_home/config/prod/application.yml level "$cluster_level" 1
    ## server.port
    replace $app_home/config/prod/application.yml port "$server_port" 1
    ## datasource
    ...
    ## Eureka
    replace $app_home/config/prod/application-eureka.yml register-with-eureka "$eureka_regist" 1
    replace $app_home/config/prod/application-eureka.yml fetch-registry "$eureka_fetch" 1
    replace $app_home/config/prod/application-eureka.yml defaultZone "$eureka_service_url" 1
    if [ ! -z "$eureka_instance_id" ];then
        replace $app_home/config/prod/application-eureka.yml instance-id "$eureka_instance_id" 1
    fi
    ## Apollo
    replace $app_home/config/prod/application-apollo.yml enabled "$apollo_enabled" 1
    replace $app_home/config/prod/application-apollo.yml meta "$apollo_meta" 1
    replace $app_home/config/prod/application-apollo.yml cluster "$apollo_cluster" 1
    replace $app_home/config/prod/application-apollo.yml namespaces "$apollo_namespaces" 1
}
